...
test_full = 'TestColorFull' in sys.argv[-1] or bool(os.environ.get('TEST_COLOR_FULL'))


class Abstract:
    # Nest abstract case suite to avoid its detection during discovery
    class BaseCase(TestCase, metaclass=ABCMeta):
        @staticmethod
        @abstractmethod
...
@skipIf(test_full, 'full suite is running')
class TestColorSample(Abstract.BaseCase):
    @staticmethod
    @override
    def values() -> Iterable[int]:
...
@skipUnless(test_full, 'must be called directly or with TEST_COLOR_FULL env specified')
class TestColorFull(Abstract.BaseCase):
    @staticmethod
    @override
    def values() -> Iterable[int]:
        return range(256)
